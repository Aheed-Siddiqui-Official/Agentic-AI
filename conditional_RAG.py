import os
# used when context specific data or need a data structure to store data
...
class State(TypedDict):
    programme : str # bca, bcom, bba
    messages : Annotated[list, add_messages] # [human : question] router node tells conditional edge where to go, classifier node tells which node question it is fees, academic or general
...
def academic_rag_node(state: State) -> dict:
    ...
    # send a query using similarity search and the chunks will be saved in the docs
...
def fee_rag_node(state: State) -> dict:
    ...
    query = state["messages"][-1].content
    # docs will be retrieved from fee retriever
